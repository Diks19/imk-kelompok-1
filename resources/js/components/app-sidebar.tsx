...
export function AppSidebar() {
    ...
    return (
        <aside 
            className={`flex flex-col bg-zinc-950 border-r border-zinc-800 transition-all duration-300 h-screen shrink-0 ${
        ...
        >
            ...
            <div className="p-4 flex items-center justify-between">
                {!isCollapsed && (
                    <Link href="/" className="flex items-center gap-2 px-2">
                        ...
                        <span className="text-zinc-100 font-bold tracking-tight text-xl text-nowrap">LarAI</span>
                    </Link>
                )}
                <button 
                    onClick={toggleCollapse}
                    className={`p-2 hover:bg-zinc-900 rounded-lg text-zinc-400 transition-colors ${isCollapsed ? 'mx-auto' : ''}`}
                >
            ...
            <div className="px-4 py-2">
                <Link
                    href="/chat"
                    className={`flex items-center gap-3 bg-purple-600 hover:bg-purple-500 text-white rounded-xl transition-all h-12 ${
            ...
            <nav className="flex-1 overflow-y-auto px-4 py-4 space-y-8 scrollbar-hide">
                <div>
                    {!isCollapsed && (
                        <h3 className="px-4 text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-4">
                            History
                        </h3>
                    )}
                        ...
                        {conversations.map((conv) => (
                            ...
                            >
                                <MessageSquare className={`size-5 shrink-0 ${
                                     url === `/chat/${conv.id}` ? 'text-purple-500' : ''
                                }`} />
                                {!isCollapsed && (
                                    <>
                                        ...
                                        <div className="absolute right-3 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            <button
                                                ...
                                                className="p-1 rounded-md hover:bg-red-500/20 text-zinc-400 hover:text-red-500 transition-colors"
                                                title="Delete Chat"
                                            >
            ...
            <div className="p-4 border-t border-zinc-900 space-y-1">
                <Link
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl text-zinc-400 hover:bg-zinc-900 hover:text-zinc-100 transition-all ${
                        isCollapsed ? 'justify-center w-12 mx-auto' : ''
                    }`}
                >
                ...
                <Link
                    ...
                    method="post"
                    as="button"
                    className={`flex items-center gap-3 p-3 rounded-xl text-zinc-400 hover:bg-red-950/30 hover:text-red-400 transition-all w-full text-left ${
                        isCollapsed ? 'justify-center w-12 mx-auto' : ''
                    }`}
                >
